...
| in65     | float64  | number    | ![Alt text](https://github.com/ASainio/WISE-Light-Curves/blob/main/null-hyph.png) 65 of 0hypothesis values are first counted from mean magnitude of filtered data. Then Baseline is estimated by finding right magnitude where 0-hypothesis value covers most of datapoints. Datapoints are counted and expressed as %. These values can be used to determine if source is variable and also give hint of the type of variability (dimming or brightening comapared to baseline). |
